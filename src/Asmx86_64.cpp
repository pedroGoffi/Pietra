...
void makeLabel(int addr) { printlb(".L%i", addr); }
void makeLabel() { makeLabel(count()); }
...
Type *compile_eq(Expr *lhs, Expr *rhs, State::CompilerState &state) {
  ...
  if (lhs_t->name) {
    if (Sym *sym = sym_get(lhs_t->name)) {
      // NOTE:
      /*
          if method __eq__ exist the compiler will call it even if
         struct_reassigner exists struct_reassigner will only be called if:
      ...
      */
      if (sym->kind == Resolver::SYM_AGGREGATE) {
        if (Sym *__eq = sym->impls.find("__eq__")) {
          ...
          // NOTE: this will be compiled to struct_reassign(lhs, rhs,
          // sizeof(lhs))
          return compile_expr(call, State::CompilerState::GetAddress());
        } else {
          // Call std.memcpy
          err("[ERROR]: Struct `%s` can't be assigned directly without "
              "prep-proc: `%s` or method implementation __eq__.\n",
              sym->type->repr(), STRUCT_REASSIGN_FLAG);
          exit(1);
        }
      }
    }
...
Type *compile_binary(TokenKind kind, Ast::Expr *lhs, Ast::Expr *rhs,
  ...
  case TK_eq:
    return compile_eq(lhs, rhs, state);
...
Type *lea(Variables::Variable *var) {
  if (var->isGlobal) {
    ...
    println("lea rax, %s [rbp - %i]", get_word_size(var->type->size),
            var->stackOffset);
    return type_ptr(var->type, var->type->ismut);
  }
}
...
Type *mov(Variables::Variable *var) {
  if (var->isGlobal) {
    ...
    println("mov rax, %s [rbp - %i]", get_word_size(var->type->size),
            var->stackOffset);
    return type_ptr(var->type, var->type->ismut);
  }
}
...
void store() {
  ...
  if (type->size == 1) {
    ...
    println("mov %s [rdi], %s\n", word, rax_reg_from_size(type->size));
  }
}
...
void store(Type *lhs, Type *rhs) {
  store();
  return;
  if (lhs->kind == rhs->kind and
      (lhs->kind == Ast::TYPE_STRUCT or lhs->kind == Ast::TYPE_UNION)) {
    ...
    for (TypeField *&field : lhs->aggregate.items) {
      println(";; NOTE: Assign struct %s, in field [size=%i] %s", lhs->name,
              field->type->size, field->name);
      // var rdi
      // val rax
      if (offset > 0) {
      ...
      {
        println("mov %s [rdi], rcx", get_word_size(field->type->size));
        println("add rdi, %i\n", field->type->size);
      }
...
void compile_aggregate_constructor(Variables::Variable *var, Sym *constructor) {
  ...
  Expr *e = Utils::expr_field_access(
      {}, base, Utils::expr_name({}, AGGREGATE_CONSTRUCTOR_WORD));
  compile_expr(e, State::CompilerState::None());
  println("call rax");
}

Type *compile_init_var(const char *name, Type *type, Expr *init, bool isGlobal,
                       State::CompilerState &state) {
  assert(!isGlobal &&
         "global variables should be preprocessed in the resolver");
  ...
  if (var->type->kind != TYPE_PTR) {
    ...
    if (sym and sym->type->kind != TYPE_PTR and
        sym->kind == Resolver::SYM_AGGREGATE and sym->type->name) {
  ...
  if (init) {
    Expr *eq = Utils::expr_binary(init->loc, TK_eq,
                                  Utils::expr_name(init->loc, var->name), init);
    compile_expr(eq, State::CompilerState::None());
  }
  return var->type;
}

bool check_arg_with_param(Type *arg, Type *param) {
  return arg->typeCheck(param);
}
void compile_call_arg(Type *proc_t, Expr *arg, int arg_index) {
  assert(proc_t->isCallable());
  int proc_params_size = proc_t->proc.params.len();

  if (arg_index > proc_params_size) {
    if (!proc_t->proc.is_vararg) {
      err("[ERROR]: error while calling %s, expected %i arguments\n",
          proc_t->name, proc_params_size);
      exit(1);
    }

    // the proc_t is vararg and we gonna push in stacky :3
    Type *arg_t = compile_expr(arg, State::CompilerState::None());
    push("rax", arg_t);
    return;
  }

  Type *arg_t = compile_expr(arg, State::CompilerState::None());
  TypeField *param_tf = proc_t->proc.params.at(arg_index);

  if (!check_arg_with_param(arg_t, param_tf->type)) {
    err("[ERROR]: expected type %s but got %s in n-%i argument in %s\n",
        param_tf->type->repr(), arg_t->repr(), arg_index, proc_t->name);
    exit(1);
  }

  Type *t = pop_nostack("rax");
  mov_to_rax(t, reg_from_size(t->size, arg_index));
  push("rax", arg_t);
  return;
}
Type *compile_call_sizeof(Expr *base, SVec<Expr *> args, State::CompilerState &state) {  
  assert(args.len() == 1);
  Expr *arg = args.at(0);
  ...
  Expr *e;
  if (sym->kind == Resolver::SYM_TYPE or
      sym->kind == Resolver::SYM_AGGREGATE or
      sym->kind == Resolver::SYM_VAR) {
...
Type *compile_call_asm(Expr *base, SVec<Expr *> args, State::CompilerState &state){
...
Type *compile_call_syscall(Expr *base, SVec<Expr *> args, State::CompilerState &state){
  int len = args.len();
  if (len > 1) {
...
void propagate_call_args(Type* base_t, SVec<Expr*>& args, bool isVa=false, int vaPos=-1){
  int id = 0;
  for (Expr *arg : args) {  
    ...
    if (arg->kind == EXPR_NAME) {
      if (Sym *sym = sym_get(arg->name)) {
        if (sym->type->kind == Ast::TYPE_STRUCT or
            sym->type->kind == Ast::TYPE_UNION) {
  ...
  for (Expr *arg : args) {
    ...
    if (isVa and (argPos >= vaPos)) {
      for (int i = args.len() - 1; i >= vaPos - 1; i--) {
        Expr *va_arg = args.at(i);
        Type *arg_t = compile_expr(va_arg, State::CompilerState::None());
        push("rax", arg_t);
      }
      break;
    }
...
Type *compile_call_from_context(Expr *base, SVec<Expr *> args, State::CompilerState &state) {
  ...
  if (!isVa) {
    ...
    println("push __VA_END__");
  }
  
  propagate_call_args(base_t, args, isVa, vaPos);
...
Type *compile_call(Expr *base, SVec<Expr *> args, State::CompilerState &state) {
  if (base->kind == EXPR_NAME) {
    if (base->name == keyword_asm)                        return compile_call_asm(base, args, state);
    else if (base->name == Core::cstr("sizeof"))      return compile_call_sizeof(base, args, state);
    else if (base->name == keyword_syscall)               return compile_call_syscall(base, args, state);
  }
  return compile_call_from_context(base, args, state);
}
...
Type *compile_name(const char *name, State::CompilerState &state) {
  ...
  if (proc) {
    ...
    if (Variables::Variable *local = proc->findLocalVariable(name)) {
      makeLabel();
      lea(local);
      // TODO: Load if the state is not get_addr and if the local variable is
      // not a non-pointer to a struct object
      if (state != State::CompilerState::GetAddress()) {
        load(local->type);
      }
...
Type *compile_field(Expr *parent, Expr *children, State::CompilerState &state) {
  ...
  if (children->kind == Ast::EXPR_NAME) {
    ...
    err("target = %s\n", p->name);
    err("[ERROR]: field %s is not existent in the object %s.", children->name,
        p->repr());
    exit(1);
  }
...
Type *compile_expr_switch(Expr *e, State::CompilerState &state) {
  ...
  for (SwitchCase *c : cases) {
    ...
    for (SwitchCasePattern *pt : c->patterns) {
      if (pt->begin != pt->end) {
        Expr *lt = Utils::expr_binary({}, TK_less,
                                      Utils::expr_int({}, pt->begin), cond);

        Expr *gt = Utils::expr_binary({}, TK_greater,
                                      Utils::expr_int({}, pt->begin), cond);
...
Type *compile_lambda(Expr *&e) {
  ...
  Expr *call =
      Utils::expr_call(e->loc, Utils::expr_name(e->loc, lambda->name), args);

  return compile_expr(Utils::expr_name(e->loc, lambda->name),
                      State::CompilerState::None());
}
Type *compile_new(Expr *&e) {
  if (!new_allocator) {
    err("[ERROR]: `new` allocator not configured in the compiler, compilation "
        "will be aborted.\n");
    exit(1);
  }
  ...
  */
  SVec<Expr *> call_args;
  /* rdi -> */ call_args.push(
      Utils::expr_int(e->loc, rty->size)); // item nbytes
  /* rsi -> */ call_args.push(size);       // list size

  Expr *new_call =
      Utils::expr_call(e->loc, Utils::expr_name(e->loc, s->name), call_args);
  Type *ptr_ty = compile_expr(new_call, State::CompilerState::None());
  if (args.len() == 0)
    return ptr_ty;
  if (type->kind != Ast::TYPESPEC_NAME) {
    err("[ERROR]: Compiling typespec in `new` operator expects type name, got: "
        "%s.\n",
  ...
  if (!method) {
    compiler_error(e->loc, "[ERROR]: trying to pass arguments in new operator "
                           "for a struct with no constructor\n");
    // err("[ERROR]: trying to pass arguments in new operator for a struct with
    // no constructor\n");
    exit(1);
  }
  ...
  if (method->decl->proc.params.len() != args.len() + 1) {
    ...
    int got = args.len();
    err("[ERROR]: Trying to call constructor of `%s` that expects %i arguments "
        "but just got %i\n",
        ptr_ty->repr(), expected, got);
    exit(1);
  }

  // stack = ... ptr
  for (Expr *&arg : args) {
    /* rax -- nth-arg */ Type *arg_ty =
        compile_expr(arg, State::CompilerState::None());
...
Type *compile_expr(Expr *e, State::CompilerState &state) {
  ...
  case EXPR_NAME:
    return compile_name(e->name, state);
  case EXPR_UNARY:
    return compile_unary(e->unary.unary_kind, e->unary.expr, state);
  case EXPR_BINARY:
    return compile_binary(e->binary.binary_kind, e->binary.left,
                          e->binary.right, state);
  case EXPR_INIT_VAR:
    return compile_init_var(e->init_var.name, e->init_var.type->resolvedTy,
                            e->init_var.rhs, false,
                            State::CompilerState::None());
  case EXPR_SWITCH:
    return compile_expr_switch(e, state);
  case EXPR_CALL:
    return compile_call(e->call.base, e->call.args, state);
  case EXPR_CAST:
    return compile_cast(e->cast.typespec, e->cast.expr, state);
  case EXPR_ARRAY_INDEX:
    return compile_index(e->array.base, e->array.index, state);
  case EXPR_FIELD:
    return compile_field(e->field_acc.parent, e->field_acc.children, state);
  case EXPR_TERNARY:
    return compile_ternary(e->ternary.cond, e->ternary.if_case,
                           e->ternary.else_case);
  case EXPR_LAMBDA:
    return compile_lambda(e);
  case EXPR_NEW:
    return compile_new(e);
  default:
    fprintf(stdout, "========[ ERROR PLANG UNKNOWN EXPRESSION COMPILATION "
                    "BRANCH ]================\n");
...
Type *compile_is_between(Expr *cond, int begin, int end) {
  ...
  Expr *low = Utils::expr_binary(cond->loc, TK_less, cond,
                                 Utils::expr_int(cond->loc, begin));
  Expr *high = Utils::expr_binary(cond->loc, TK_greater, cond,
                                  Utils::expr_int(cond->loc, end));
    ...
    for (auto *p : patterns) {
      if (p->begin != p->end) {
        ...
        if (p->name) {
            ...
            if (d_enum->kind == Ast::DECL_CONSTEXPR) {
              ...
              if (e->kind == EXPR_INT) {
                ...
                if (!string_comparator) {
                  err("[WARN]: Didn't found any string comparator procedure "
                      "defined with @string_comparator.\n");
                  exit(1);
                }
                ...
                Expr *str_cmp_expr = Utils::expr_call(
                    e->loc, Utils::expr_name(e->loc, scmp->decl->name), args);
          ...
          if (!string_comparator) {
            err("[WARN]: Didn't found any string comparator procedure defined "
                "with @string_comparator.\n");
          ...
          Expr *str_cmp_expr = Utils::expr_call(
              {}, Utils::expr_name({}, scmp->decl->name), args);
        ...
        assert(p->name);
        Sym *sym = sym_get(p->name);
        if (Constants::ConstantExpression *ce =
                ctx.getConstantExpression(sym->name)) {
...
void compile_stmt(Stmt *stmt) {
  ...
  case Ast::STMT_IF: {
    int if_end = count();
    {
      Type *cond = compile_expr(stmt->stmt_if.if_clause->cond,
                                State::CompilerState::None());
      cmp_zero(cond);
  ...
  case Ast::STMT_WHILE: {
    ...
    if (stmt->stmt_while->is_doWhile) {
      ...
      Type *ty =
          compile_expr(stmt->stmt_while->cond, State::CompilerState::None());
      cmp_zero(ty);
      println("je .L%i", end);
...
void compile_variable_deleter(Variables::Variable *var, Sym *del) {
  ...
  err("deleter of %s\n", var->name);
  Expr *e =
      Utils::expr_field_access({}, Utils::expr_name({}, var->name),
...
void compile_decl_proc(Decl *d) {
  ...
  assert(proc->parameters->len() < MAX_ARGREG);
  {
    ...
    for (Variables::Variable *param : *proc->parameters) {
      ...
      if (param->type->kind != TYPE_PTR and
          (param->type->kind == TYPE_STRUCT or
           param->type->kind == Ast::TYPE_UNION)) {
        ...
        if (Sym *del = impls.find(AGGREGATE_DELETER_WORD)) {
          ...
          if (0) {
            for (TypeField *tf : t->aggregate.items) {
              ...
              if (Sym *child_killer =
                      child_sym->impls.find(AGGREGATE_DELETER_WORD)) {
                if (stack_offset > 0) {
                  lea(local);
                ...
                Variables::Variable *child_var = new Variables::Variable{
                    child_sym->name, child_sym->type, nullptr,
                    false,           false,           stack_offset};
...
void compile_ast(SVec<Decl *> ast, COMPILER_TARGET target,
  ...
  println("mov rdi, 0");
  println("syscall");
  println("mov [__heap_end__], rax") for (Variables::Variable *global :
                                          ctx.globalVariables) {
    ...
    const char *name = global->type->kind == TYPE_PTR ? global->type->base->name
                                                      : global->type->name;
  ...
  if (DEBUG_MODE) {
    ...
    case CT_LINUX: {
      ...
      system(strf(
          "ld -lc -dynamic-linker /lib64/ld-linux-x86-64.so.2 pietra.o -o %s",
          output_file));
      system(strf("rm pietra.o"));
      break;
    }
    case CT_WINDOWS: {
      printf("[WARN]: compiler target as windows hasn't been deployed yet, "

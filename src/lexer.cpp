...
const char *check_not_intern(const char *str) {
  for (const char **intern : intern_table) {
    if (*intern == str)
      return strf("keyword '%s' is internal.", *intern);
  }
  return nullptr;
}
...
static Token makeToken(Lexer *lexer, const char *fc, const char *lc,
                       TokenKind kind, long i64) {
  return {.firstChar = fc,
          .lastChar = lc,
          .kind = kind,
...
static Token makeToken(Lexer *lexer, const char *fc, const char *lc,
                       TokenKind kind, const char *string) {
  return {.firstChar = fc,
          .lastChar = lc,
          .kind = kind,
          .name = cstr_range(fc, lc),
...
static Token makeTokenPonctuation(Lexer *lexer, const char *fc, const char *lc,
                                  TokenKind kind) {
...
const bool Token::isKind(TokenKind kind) const { return this->kind == kind; }
...
const bool Lexer::isIdentifier() const {
  return (
      (*this->stream.parsePoint >= 'a' and *this->stream.parsePoint <= 'z') or
      (*this->stream.parsePoint >= 'A' and *this->stream.parsePoint <= 'Z') or
      (*this->stream.parsePoint == '_'));
}
...
void Lexer::skipWhiteSpaces() {
  for (;;) {
    while (this->stream.parsePoint != this->stream.eof) {
      ...
      if (this->stream.eof and
          this->stream.parsePoint + increment > this->stream.eof) {
    ...
    // Skip comments
    if (!this->isEof() and this->stream.parsePoint[0] == '/' and
        this->stream.parsePoint[1] == '/') {

      while (this->stream.parsePoint != this->stream.eof and
             *this->stream.parsePoint != '\r' and
...
double Lexer::scanFloat() {
  ...
  if (tolower(*this->stream.parsePoint) == 'e') {
    ...
    if (*this->stream.parsePoint == '+' or *this->stream.parsePoint == '-')
      this->advanceChar();
  ...
  if (tolower(*this->stream.parsePoint) == 'f')
    this->advanceChar();
...
const char *Lexer::scanString() {
  assert(
      (*this->stream.parsePoint == '"' or *this->stream.parsePoint == '\'') and
      "Expected single quotes or double quotes when scanning a token string");
  ...
  if (this->isEof())
    syntax_error(this->getLocation(),
                 "Expected '\"' after string body, got EOF.\n");
...
Token Lexer::getToken() { return this->token; }
...
Token Lexer::lexNextToken() {
  ...
  default: {
    ...
    if (this->isIdentifier()) {
      do
        this->advanceChar();
      while (!this->isEof() and (this->isIdentifier() or this->isNumber()));
  ...
  case '.': {
    ...
    if (!this->isEof() and this->stream.parsePoint + 1 != this->stream.eof) {
      if (this->stream.parsePoint[0] == '.' and
          this->stream.parsePoint[1] == '.') {
        kind = TK_tripledot;
        this->advanceChar();
        this->advanceChar();
  ...
  case '"': {
    ...
    return makeToken(this, begin, this->stream.parsePoint - 1, TK_dqstring,
                     string);
  }
  case '\'': {
    const char *string = this->scanString();
    return makeToken(this, begin, this->stream.parsePoint - 1, TK_sqstring,
                     string);
  }
  case '0':
  case '1':
...
void Lexer::parsingTemplateContext() { this->parsingTypeSpecTemplate = true; }
void Lexer::expectSemiColon() {
  if (!this->expect(TK_semicolon))
    syntax_error(this->getLocation(), "Expected ';'.\n");
}
...
const char *default_paths[] = {
    ...
    "/home/pietra/workspace/" // default std path
};
Lexer *Lexer::open_at_file(const char *file_path) {
  for (const char *&pietra_path : default_paths) {
    const char *build_path = strf("%s%s", pietra_path, file_path);
    const char *file_data = fileReader::read_file(build_path);
    if (file_data) {
      // Found item to read
      Lexer *lexer = new Lexer(file_data);
      lexer->loc = SrcLocation(build_path, 1);
      return lexer;
    }
  }
...
void resolver_log(SrcLocation loc, const char *fmt, ...) {
  ...
  va_list args;
  va_start(args, fmt);
  printf("%s (%d: %d): resolver log: ", loc.name, loc.lineNumber,
         loc.lineOffset);
  vprintf(fmt, args);
  va_end(args);
}

void setResolverDebug(bool state) { resolver_debug = state; }
...
void include_me(const char *str) { included_packages.push(Core::cstr(str)); }

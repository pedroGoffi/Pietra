...
#include <string>
namespace Pietra {
void pietra_run_compilation_on_input_file(bool doLog, std::string input_file,
...
std::string get_or_create_output_file_name(std::string input_file,
                                           std::string output_file);
void initialize_language_context();
int process_rtpi_file(const std::string &input_file,
                      const std::string &output_file, bool log,
                      const ArgumentParser &parser);
int process_pi_file(const std::string &input_file, std::string &output_file,
                    Asm::COMPILER_TARGET target, bool log);
int initialize_and_parse_arguments(int argc, char **argv,
                                   ArgumentParser &parser, bool &verbose,
                                   Asm::COMPILER_TARGET &target,
...
int pietra_process_input_file(const std::string &input_file,
                              std::string &output_file, bool log,
...
void pietra_setup_argparser(ArgumentParser &parser) {
  ...
  parser.add_argument("-win32",
                      "Set the default output for binary code to win32",
                      true);                               // flag argument
  parser.add_argument("--file", "Specify the input file"); // non-flag argument
  parser.add_argument(
      "--output", "Specify the output file"); // optional output file argument
  parser.add_argument("--verbose", "Verbose prints in compilations steps",
                      true);
  parser.add_argument("--target", "Specify LLVM or Goffi compiler");
}
int pietra_check_arg_parser(ArgumentParser &parser, int argc, char **argv) {
  // Add arguments to the parser
  if (argc < 2) {
    parser.show_help();
    fprintf(stderr,
            "[ERROR]: epected input file %s <INPUT FILE> <OPTIONAL FLAGS>.\n",
            argv[0]);
    return 0;
  }
...
int pietra_run_preprocess_on_input_file(bool log, std::string input_file) {
  ...
  all_start = clock();
  if (log)
    fprintf(stderr, "Initializing pietra compiler on %s\n", input_file.c_str());
...
std::string get_or_create_output_file_name(std::string input_file,
  ...
  if (output_file.empty()) {
    // If no output file provided, create the default output file name in the
    // current directory Extract the filename from the input path (remove
    // subfolders)
    std::filesystem::path input_path(input_file);
    std::string filename =
        input_path.filename().string(); // Get the filename without subfolders
...
int process_rtpi_file(const std::string &input_file,
  ...
  if (!output_file.empty()) {
    ...
    fprintf(
        stderr,
        "[ERROR]: Output file must not be specified for preprocess files.\n");
    return EXIT_FAILURE;
  }
...
int process_pi_file(const std::string &input_file, std::string &output_file,
                    Asm::COMPILER_TARGET target, bool verbose,
                    std::string target_kind) {
...
int initialize_and_parse_arguments(int argc, char **argv,
  ...
  target = parser.is_flag_set("-win32") ? Asm::COMPILER_TARGET::CT_WINDOWS
  ...
  target_kind = parser.get_value("--target");
  return EXIT_SUCCESS;
}
...
void log_duration(const std::string &action, clock_t start, clock_t end) {
  ...
  if (log)
    fprintf(stderr, "> []: Time spent in %s: %.6f seconds.\n", action.c_str(),
            duration);
}
...
void pietra_run_compilation_on_input_file(bool doLog, std::string input_file,
  ...
  start = clock();
  if (target_kind == "llvm") {
    // compile_ast_to_llvm(ast, target, output_file.c_str(), verbose);
  } else {
...
int Main(int argc, char **argv) {
  ...
  std::string input_file, output_file, target_kind;

  if (initialize_and_parse_arguments(argc, argv, parser, verbose, target,
                                     input_file, output_file,
                                     target_kind) == EXIT_FAILURE) {
    return EXIT_FAILURE;
  }

  initialize_language_context();
  if (pietra_process_input_file(input_file, output_file, verbose, parser,
                                target, target_kind) == EXIT_FAILURE) {
